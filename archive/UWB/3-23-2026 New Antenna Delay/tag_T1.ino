...
//
// Revision: Single-response protocol — anchor sends one frame
//           containing both the timing reference (T4) and the
//           40-bit reply delay (T3-T2).  Antenna delay = 16800.
// ============================================================
...
void loop() {
  ...
  // Response frame: [0]=MSG_RESPONSE [1]=anchorId [2..6]=replyDelay (5 bytes LE)
  if (rxBuffer[0] != MSG_RESPONSE || len < 7) {
    Serial.print(F("T")); Serial.print(DEVICE_ID);
    Serial.print(F(" #")); Serial.print(rangeSeq);
    Serial.println(F("  BAD RESP"));
    return;
  }

  uint8_t anchorId = rxBuffer[1];

  // Extract full 40-bit reply delay from anchor (little-endian, 5 bytes at offset 2)
