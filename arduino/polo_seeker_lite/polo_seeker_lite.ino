// ============================================================
// POLO SEEKER LITE v7.2 — TWR Initiator + RSSI + JSON Telemetry
// Board: Arduino Nano 33 BLE Sense + DWM1000
// (GPS CODE REMOVED FOR SUB-TRACKERS)
// ============================================================
...
// ============================================================
// ⚠️ EDIT THESE PINS TO MATCH YOUR SUPER-GLUED WIRING ⚠️
// ============================================================
const uint8_t PIN_CS  = 10; // Chip Select (CSN)
const uint8_t PIN_IRQ = 2;  // Interrupt (IRQ)
const uint8_t PIN_RST = 3;  // Reset (RSTN)
// ============================================================
...
void setup() {
  ...
  Serial.println("\n--- BOOTING SEEKER LITE v7.2 (NO GPS) ---");
  ...
  Serial.println("Seeker Lite v7.2 ready. Ranging every 1s.\n");
}
...
void loop() {
  ...
  // ***** READ RSSI AND CLOCK OFFSET HERE *****
  ...
  if (carrierIntegrator & 0x00100000) { 
  ...
  Serial.print("  co=");
  Serial.print(clockOffsetRatio * 1e6, 2); 
  Serial.print("  d=");
  Serial.print(dist, 3);
  Serial.println("m");
...
void emitSeekerJson() {
  ...
  JsonDocument doc;
  doc["id"]  = "seeker_1"; // (This is overridden by the Python node anyway)
